...
# Patterns to detect AI refusals
REFUSAL_PATTERNS = [
    # General refusals
...
async def call_openrouter_api(
    ...
    """
    Call OpenRouter API with retry logic and refusal detection.
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    return f"⚠️ Error: {response.status}"
...
async def generate_compliance_analysis(product: str, country: str) -> str:
    """
    Generate compliance analysis for a product in a specific country.
